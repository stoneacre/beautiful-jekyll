...
function replaceTags(html, data) {
  ...
  html = html.replace(/%%card_annual_fee%%/g, data[22]);
  html = html.replace(/%%card_balance_transfer_fee%%/g, data[23]);
  html = html.replace(/%%card_foreign_transaction_fee%%/g, data[24]);
  html = html.replace(/%%card_rewards_rate%%/g, data[25]);
  html = html.replace(/%%card_introductory_reward%%/g, data[27]);
